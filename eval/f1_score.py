...
def exon_metrics(ref_gff, pred_gff):
    """Calculate exon-level metrics — both boundaries must match exactly"""
...
def gene_metrics(ref_gff, pred_gff):
    """Calculate gene-level metrics — complete gene structure must match"""
...
def calculate_f1(ref_gff, pred_gff, level="exon"):
    """Calculate F1 score at specified level (nucleotide, exon, or gene)"""
