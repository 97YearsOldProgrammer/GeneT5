...
tokenizer = tk.KmerTokenizer(k=arg.kmer, stride=arg.stride, vocabulary=vocabulary)
print(f"\nTokenizer: {len(vocabulary):,} k-mers (k={arg.kmer}, stride={arg.stride})")

# Tokenize everything
print(f"\nTokenizing...")
all_tokens = ds.tokenize_all(tokenizer)

with open(arg.output, 'w') as f:
    for tokens in all_tokens:
        f.write(' '.join(str(t) for t in tokens) + '\n')

print(f"Written {len(all_tokens):,} sequences to {arg.output}\n")