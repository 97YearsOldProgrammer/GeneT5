"""Dynamic chunking logic for genomic sequences"""
...
def generate_hints_from_features(features, noise_rate=0.1):
    ...
    for feat in features:
        ...
        jitter_start = int(rnd.gauss(0, 15))
        jitter_end   = int(rnd.gauss(0, 15))
    ...
    if rnd.random() < 0.05 and features:
        max_pos    = max(f["end"] for f in features)
        fake_start = rnd.randint(0, max(0, max_pos - 200))
...
def augment_with_hints(chunks, hint_ratio=0.5, seed=42):
    ...
    indices        = rnd.sample(range(len(chunks)), min(num_to_augment, len(chunks)))
