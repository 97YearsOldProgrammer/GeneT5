...
class MoE(nn.Module):
    ...
    def __init__(self, config, force_backend=None):
        super().__init__()
        self.config = config
        ...
        if force_backend == "triton":
            ...
            self.backend = "triton"
            self.moe     = TritonMoEBackend(config)
            
        elif force_backend == "mps":
            self.backend = "mps"
            self.moe     = MPSMoEBackend(config)
        ...
        print(f"MoE initialized with backend: {self.backend}")
    ...
    def _get_buffers(self, num_tokens, capacity, device, dtype):
        ...
        if (self._dispatch_buffer is None or 
            ...
            self._combine_buffer = torch.zeros(
                total_k, self.embed_dim, device=device, dtype=dtype
            )
...
if TRITON_AVAILABLE:
    ...
    class TritonMoEBackend(nn.Module):
